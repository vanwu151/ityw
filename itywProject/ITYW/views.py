    ...
    if request.session.get('is_login', None):
        if request.method == "POST":
            ...
            try:
                ...
                item_change_info = i_i.objects.get( item_sn = item_sn ).item_info
            ...
            if action == '新增':
                ...
                try:
                    ...
                    try:
                        s = i_c_i( item_change_date = item_change_date, item_name = item_name, 
                            item_sn = item_sn, item_pass_user = item_now_user ,item_now_user = item_now_user,
                            item_now_user_workid = item_now_user_workid, item_change_info = item_change_info,
                            item_change_location =  item_change_location, item_statu = item_statu, 
                            item_pass_info = item_pass_info, change_info_user = name, add_item_info = item_add_info)
                        s.save()
                        info = '{} ，编号/号码 {} 已添加'.format(item_sn, item_add_info)
                    except Exception as e:
                        print('{}新增过程中出现异常：'.format(item_add_info),e)
                ItemDetailDataView = getItemDetail( name = name, item_sn = item_sn )
                ItemDetailData = ItemDetailDataView.getItemDetailData()
                ItemDetailData['userinfo']['info'] = info
                ItemDetailData['userinfo']['Lmj_info_list'] = Lmj_ii.item_info
            ...
            if action == '删除':
                ...
                try:
                    ...
                    s = i_c_i( item_change_date = item_change_date, item_name = item_name, 
                        ...
                        item_now_user_workid = item_now_user_workid, item_change_info = item_change_info,
                        item_change_location =  item_change_location, item_statu = item_statu, 
                        item_pass_info = item_pass_info, change_info_user = name, del_item_info = item_list_info)
                    s.save()
                    info = '{} ，编号/号码 {} 已删除'.format(item_sn, item_list_info)
                    ItemDetailDataView = getItemDetail( name = name, item_sn = item_sn )
                    ItemDetailData = ItemDetailDataView.getItemDetailData()
                    ItemDetailData['userinfo']['info'] = info
                    ItemDetailData['userinfo']['Lmj_info_list'] = Lmj_ii.item_info
